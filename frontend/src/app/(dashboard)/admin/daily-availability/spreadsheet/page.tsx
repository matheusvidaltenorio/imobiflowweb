...
export default function SpreadsheetImportPage() {
  ...
  const [saveGestora, setSaveGestora] = useState('');
  ...
  const { data: templates } = useQuery({
    queryKey: ['spreadsheet-templates', developmentId],
    queryFn: async () => {
  ...
  const { data: importHistory } = useQuery({
    queryKey: ['daily-availability-history', developmentId, 'spreadsheet'],
    queryFn: async () => {
      const { data } = await api.get<
        Array<{
          id: string;
  ...
  const confirmMutation = useMutation({
    mutationFn: async () => {
      ...
      fd.append('columnMapping', JSON.stringify(mapping));
      if (saveTplName.trim()) {
        fd.append('saveTemplateJson', JSON.stringify({ name: saveTplName.trim(), gestoraLabel: saveGestora.trim() || undefined }));
      }
      if (activeTemplateId) {
    ...
    <main className="p-6 md:p-10">
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        >
          ...
          Voltar à central de disponibilidade
        </Link>
        ...
        {preview ? (
          <Card className="space-y-4 p-6">
            ...
            <div className="space-y-2 rounded-lg border border-surface-muted p-4">
              <Label className="text-xs">Salvar mapeamento como template (opcional)</Label>
              <Input
                placeholder="Nome do template (ex.: Planilha gestora XYZ)"
                value={saveTplName}
                onChange={(e) => setSaveTplName(e.target.value)}
              />
              <Input placeholder="Nome da gestora (opcional)" value={saveGestora} onChange={(e) => setSaveGestora(e.target.value)} />
            </div>
