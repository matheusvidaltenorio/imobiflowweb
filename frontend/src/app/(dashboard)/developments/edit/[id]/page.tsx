...
export default function EditDevelopmentPage() {
  ...
  const { data: dev, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data as { name: string; description?: string; address?: string; city: string; neighborhood?: string };
    },
  });

  const { register, handleSubmit } = useForm<FormData>({
    resolver: zodResolver(schema),
    values: dev ? { name: dev.name, description: dev.description ?? '', address: dev.address ?? '', city: dev.city, neighborhood: dev.neighborhood ?? '' } : undefined,
  });

  const update = useMutation({
    mutationFn: (d: FormData) => api.patch(`/developments/${id}`, d),
    onSuccess: () => {
