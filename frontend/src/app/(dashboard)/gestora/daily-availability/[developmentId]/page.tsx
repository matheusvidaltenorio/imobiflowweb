...
export default function GestoraDailyAvailabilityPage() {
  ...
  const currentAccess = useMemo(() => myAccess?.find((a) => a.developmentId === developmentId), [myAccess, developmentId]);

  const currentDevLabel = useMemo(() => {
    if (!currentAccess) return null;
    return `${currentAccess.development.name} — ${currentAccess.development.city}`;
  }, [currentAccess]);

  const publishNote =
    currentAccess?.publishMode === 'PENDING_REVIEW'
      ? 'Neste loteamento, suas alterações ficam pendentes até o administrador aprovar.'
      : null;

  const { data: mapData, isLoading: mapLoading } = useQuery({
    queryKey: ['lot-map', developmentId, 'gestora-daily'],
    queryFn: async () => {
    ...
    <main className="p-6 md:p-10">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <PageHeader
          ...
          description={
            currentDevLabel
              ? `${currentDevLabel} — atualização operacional (snapshot do dia). O cadastro oficial do lote não muda.`
              : 'Carregando loteamento…'
          }
        />

        {publishNote ? (
          <p className="rounded-lg border border-amber-200 bg-amber-50/90 px-4 py-3 text-sm text-amber-950">{publishNote}</p>
        ) : null}
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <div className="rounded-xl border-2 border-primary-200 bg-white p-4 shadow-sm ring-2 ring-primary-100">
            ...
            <p className="mt-2 text-xs text-slate-600">
              Grade de lotes, clique para alternar status e salvar.
            </p>
            <p className="mt-3 text-[11px] font-medium uppercase tracking-wide text-primary-700">Você está aqui ↓</p>
          </div>
          {currentAccess?.assistedImageEnabled === false ? (
            <div
              className="rounded-xl border border-dashed border-slate-300 bg-slate-100/80 p-4 text-slate-500"
              title="Desabilitado pelo administrador"
            >
              <div className="flex items-center gap-2 text-sm font-bold text-slate-600">
                <ImageIcon className="h-4 w-4 shrink-0" />
                Imagem assistida
              </div>
              <p className="mt-2 text-xs">Indisponível para este loteamento.</p>
            </div>
          ) : (
            <Link
              href={developmentId ? `/gestora/daily-availability/${developmentId}/assisted` : '#'}
            ...
            >
              ...
              <p className="mt-2 text-xs text-slate-600">
                Envie a imagem, use zoom e template visual, confira antes de gravar.
              </p>
              <span className="mt-3 inline-block text-xs font-semibold text-primary-700">Abrir modo assistido →</span>
            </Link>
          )}
          {currentAccess?.spreadsheetImportEnabled === false ? (
            <div
              className="rounded-xl border border-dashed border-slate-300 bg-slate-100/80 p-4 text-slate-500"
              title="Desabilitado pelo administrador"
            >
            ...
            <Link
              href={developmentId ? `/gestora/daily-availability/${developmentId}/spreadsheet` : '#'}
            ...
            >
              ...
              <p className="mt-2 text-xs text-slate-600">
                Envie CSV/Excel, mapeie colunas e confirme (se habilitado pelo administrador).
              </p>
        ...
        <Card className="space-y-4 p-6">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div className="space-y-2 sm:col-span-2">
              ...
              <p className="rounded-lg border border-surface-muted bg-surface/50 px-3 py-2 text-sm font-medium text-gray-800">
